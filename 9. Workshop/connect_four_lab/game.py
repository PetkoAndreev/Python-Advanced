...
def get_player_choice_func(is_test=False):
    ...
    # choices_for_test = [1, 2, 2, 4, 3, 3, 3, 7, 4, 4, 4]  # left vertical win
    # choices_for_test = [4, 3, 3, 1, 2, 2, 2, 7, 1, 1, 1]  # right vertical win
...
def check_win_condition(board, player, row_index, column_index, win_condition_count=DEFAULT_WIN_CONDITION_COUNT):
    ...
    up_right_diagonal_values = [
        is_win_condition_value(board, row_index - d, column_index + d, player, rows_count, columns_count)
        for d in range(win_condition_count)]
    results = [all(right_horizontal_values), all(left_horizontal_values),
               all(down_vertical_values), all(up_vertical_values),
               all(down_left_diagonal_values), all(down_right_diagonal_values),
               all(up_left_diagonal_values), all(up_right_diagonal_values)
               ]
    return any(results)
