...
def play_turn(board, sign, selected_pos):
    if position_available(board, selected_pos):
        ...
        current_selected_position = input(f'Select a valid available position position [1-9]: ')
...
def play():
    ...
    while not has_won() and has_moves(turns_counter):
        current_selected_position = input(f'{setup.player_names[turns_counter % 2]}, select a position [1-9]: ')
